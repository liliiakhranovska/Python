def try_to_move(current_point, target_point, board):
    ...
    def iter_reachable_points():
        for (x, y) in [(x, y) for x in range(current_x-1, current_x + 2) for y in range(current_y-1, current_y + 2) if (x, y) != (current_x, current_y) and x > -1 and y > -1 and x < 8 and y < 8]:
